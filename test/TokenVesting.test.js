...
contract('TokenVesting', (accounts) => {
  const [deployer, beneficiary1, beneficiary2] = accounts;
  const initialReleasePercentage = new BN(1000);
  ...
  beforeEach(async () => {
    ...
    this.token.transfer(this.vesting.address, web3.utils.toWei('100', 'ether'), { from: deployer });
  });
  ...
  it('emits TokensAllocated event', async () => {
    await expectEvent.inConstruction(this.vesting, 'TokensAllocated', {
  ...
  describe('token release', () => {
    ...
    describe('on start', () => {
      it('released token should be equal to the inital release', async () => {
        ...
        const expectedReleasedTokens = {
          [beneficiary1]: allocatedTokens[beneficiary1].mul(initialReleasePercentage).div(new BN(10000)),
          [beneficiary2]: allocatedTokens[beneficiary2].mul(initialReleasePercentage).div(new BN(10000)),
        };
    ...
    describe('at half vesting period', () => {
      it('released tokens should be equal to inital amount + half remaining tokens', async () => {
        ...
        const durationPercentage = new BN(5000);
        ...
        const newlyReleaseTokens = {
          [beneficiary1]: allocatedTokens[beneficiary1]
            .sub(initialReleasedTokens[beneficiary1]).mul(durationPercentage).div(new BN(10000)),
          [beneficiary2]: allocatedTokens[beneficiary2]
            .sub(initialReleasedTokens[beneficiary2]).mul(durationPercentage).div(new BN(10000)),
        };
